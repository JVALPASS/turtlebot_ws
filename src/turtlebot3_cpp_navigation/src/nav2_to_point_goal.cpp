...
void NavToPoseClient::goal_response_callback(GoalHandleNavigateToPose::SharedPtr goal_handle) {
    if (!goal_handle) {
...
int main(int argc, char ** argv) {
    ...
    // Create an instance of the NavToPoseClient node
    auto nav_to_pose_client = std::make_shared<NavToPoseClient>(node_options);

    // Define the goal coordinates (x, y, theta)
    float goal_x = 1.0;
    float goal_y = 1.0;
    float goal_theta = 0.0;

    // Send the goal to the robot
    nav_to_pose_client->send_goal(goal_x, goal_y, goal_theta);

    // Spin the node to process callbacks
    rclcpp::spin(nav_to_pose_client);
